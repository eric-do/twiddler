...
//  $('.tweet-container').remove();
  tweetArray.forEach((tweet) => { console.log(tweet.created_at, tweet.user, tweet.message); });
  ...
  $tweetContainer.appendTo('.tweet-section');
}
...
function getFormattedTime(tweetDate) {
  return moment(tweetDate).format('MMMM Do YYYY, h:mm:ss a');
}
